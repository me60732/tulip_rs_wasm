...
use tulip_rs::indicators::volatility::{Indicator, IndicatorState, Volatility, INPUTS, OPTIONS};
...
impl VolatilityState {
    ...
    pub fn from_json(json: String) -> Result<VolatilityState, JsError> {
...
#[wasm_bindgen(js_name = "volatilityIndicator")]
pub fn volatility_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Volatility::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(
        outputs_to_js(outputs)?,
        JsValue::from(VolatilityState { inner }),
    )
}
...
#[wasm_bindgen(js_name = "volatilityMinData")]
