...
use tulip_rs::indicators::cci::{Cci, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl CciState {
    ...
    pub fn from_json(json: String) -> Result<CciState, JsError> {
...
#[wasm_bindgen(js_name = "cciIndicator")]
pub fn cci_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Cci::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(CciState { inner }))
}
...
#[wasm_bindgen(js_name = "cciMinData")]
