...
use tulip_rs::indicators::vidya::{Indicator, IndicatorState, Vidya, INPUTS, OPTIONS};
...
impl VidyaState {
    ...
    pub fn from_json(json: String) -> Result<VidyaState, JsError> {
...
#[wasm_bindgen(js_name = "vidyaIndicator")]
pub fn vidya_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Vidya::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(VidyaState { inner }))
}
...
#[wasm_bindgen(js_name = "vidyaMinData")]
