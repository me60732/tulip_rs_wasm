...
use tulip_rs::indicators::atr::{Atr, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl AtrState {
    ...
    pub fn from_json(json: String) -> Result<AtrState, JsError> {
...
#[wasm_bindgen(js_name = "atrIndicator")]
pub fn atr_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Atr::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(AtrState { inner }))
}
...
#[wasm_bindgen(js_name = "atrMinData")]
