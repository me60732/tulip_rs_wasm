...
use tulip_rs::indicators::aroonosc::{AroonOsc, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl AroonoscState {
    ...
    pub fn from_json(json: String) -> Result<AroonoscState, JsError> {
...
#[wasm_bindgen(js_name = "aroonoscIndicator")]
pub fn aroonosc_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = AroonOsc::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(
        outputs_to_js(outputs)?,
        JsValue::from(AroonoscState { inner }),
    )
}
...
#[wasm_bindgen(js_name = "aroonoscMinData")]
