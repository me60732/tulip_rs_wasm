...
use tulip_rs::indicators::marketfi::{Indicator, IndicatorState, Marketfi, INPUTS, OPTIONS};
...
pub fn marketfi_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Marketfi::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(
        outputs_to_js(outputs)?,
...
pub fn marketfi_min_data(options: Vec<f64>) -> u32 {
    ...
    Marketfi::min_data(&option_arr) as u32
}
