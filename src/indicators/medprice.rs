...
use tulip_rs::indicators::medprice::{Medprice, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl MedpriceState {
    ...
    pub fn from_json(json: String) -> Result<MedpriceState, JsError> {
...
#[wasm_bindgen(js_name = "medpriceIndicator")]
pub fn medprice_indicator(inputs: JsValue, options: Vec<f64>, optional_outputs: JsValue) -> Result<js_sys::Array, JsError> {
    let inputs = inputs_from_js(inputs)?;
    let input_arr: [&[f64]; IW] = inputs.iter().map(|v| v.as_slice()).collect::<Vec<_>>()
        .try_into().map_err(|_| JsError::new(&format!("Expected {IW} input series")))?;
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Medprice::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(MedpriceState { inner }))
}

#[wasm_bindgen(js_name = "medpriceInfo")]
pub fn medprice_info() -> JsValue { info_to_object(Medprice::INFO) }

#[wasm_bindgen(js_name = "medpriceMinData")]
pub fn medprice_min_data(options: Vec<f64>) -> u32 {
    let option_arr: [f64; OW] = options.try_into()
        .unwrap_or_else(|_| panic!("Expected {OW} options"));
    Medprice::min_data(&option_arr) as u32
}
