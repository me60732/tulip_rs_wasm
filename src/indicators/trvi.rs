...
use tulip_rs::indicators::trvi::{Indicator, IndicatorState, Trvi, INPUTS, OPTIONS};
...
impl TrviState {
    ...
    pub fn from_json(json: String) -> Result<TrviState, JsError> {
...
#[wasm_bindgen(js_name = "trviIndicator")]
pub fn trvi_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Trvi::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(TrviState { inner }))
}

/// Static metadata for TRVI.
#[wasm_bindgen(js_name = "trviInfo")]
pub fn trvi_info() -> JsValue {
    info_to_object(Trvi::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "trviMinData")]
