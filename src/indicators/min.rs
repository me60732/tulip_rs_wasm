...
const IW: usize = rust_min::INPUTS;
const OW: usize = rust_min::OPTIONS;
...
pub fn min_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = rust_min::Min::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
...
pub fn min_info() -> JsValue {
    info_to_object(rust_min::Min::INFO)
}
...
pub fn min_min_data(options: Vec<f64>) -> u32 {
    let option_arr: [f64; OW] = options
        ...
        .map_err(|_| JsError::new(&format!("Expected {OW} options")))
        .unwrap_or([0.0; OW]);
    rust_min::Min::min_data(&option_arr) as u32
}
