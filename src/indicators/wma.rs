...
/// Run the WMA indicator. Returns `[outputs, state]` as a JS array.
...
pub fn wma_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = rust_wma::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
...
#[wasm_bindgen(js_name = "wmaMinDataAccuracy")]
