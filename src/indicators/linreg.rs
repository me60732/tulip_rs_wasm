...
use tulip_rs::indicators::linreg::{Indicator, IndicatorState, Linreg, INPUTS, OPTIONS};
...
impl LinregState {
    ...
    pub fn from_json(json: String) -> Result<LinregState, JsError> {
...
#[wasm_bindgen(js_name = "linregIndicator")]
pub fn linreg_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Linreg::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(
        outputs_to_js(outputs)?,
        JsValue::from(LinregState { inner }),
    )
}
...
#[wasm_bindgen(js_name = "linregMinData")]
