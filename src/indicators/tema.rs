...
use tulip_rs::indicators::tema::{Indicator, IndicatorState, Tema, INPUTS, OPTIONS};
...
impl TemaState {
    ...
    pub fn from_json(json: String) -> Result<TemaState, JsError> {
...
/// Run the TEMA indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[period]`
#[wasm_bindgen(js_name = "temaIndicator")]
pub fn tema_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Tema::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(TemaState { inner }))
}

/// Static metadata for TEMA.
#[wasm_bindgen(js_name = "temaInfo")]
pub fn tema_info() -> JsValue {
    info_to_object(Tema::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "temaMinData")]
