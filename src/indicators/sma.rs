...
use tulip_rs::indicators::sma::{Indicator, IndicatorState, Sma, INPUTS, OPTIONS};
...
impl SmaState {
    ...
    pub fn from_json(json: String) -> Result<SmaState, JsError> {
...
#[wasm_bindgen(js_name = "smaIndicator")]
pub fn sma_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Sma::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(SmaState { inner }))
}
...
#[wasm_bindgen(js_name = "smaMinData")]
