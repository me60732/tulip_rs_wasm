...
use tulip_rs::indicators::cmo::{Cmo, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl CmoState {
    ...
    pub fn from_json(json: String) -> Result<CmoState, JsError> {
...
#[wasm_bindgen(js_name = "cmoIndicator")]
pub fn cmo_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Cmo::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(CmoState { inner }))
}
...
#[wasm_bindgen(js_name = "cmoMinData")]
