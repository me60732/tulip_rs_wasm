...
use tulip_rs::indicators::trendmode::{Indicator, IndicatorState, TrendMode, INPUTS, OPTIONS};
...
pub fn trendmode_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = TrendMode::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(
        outputs_to_js(outputs)?,
