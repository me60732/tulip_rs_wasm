...
use tulip_rs::indicators::trix::{Indicator, IndicatorState, Trix, INPUTS, OPTIONS};
...
impl TrixState {
    ...
    pub fn from_json(json: String) -> Result<TrixState, JsError> {
...
#[wasm_bindgen(js_name = "trixIndicator")]
pub fn trix_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Trix::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(TrixState { inner }))
}
...
#[wasm_bindgen(js_name = "trixMinData")]
