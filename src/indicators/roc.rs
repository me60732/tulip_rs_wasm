...
use tulip_rs::indicators::roc::{Indicator, IndicatorState, Roc, INPUTS, OPTIONS};
...
impl RocState {
    ...
    pub fn from_json(json: String) -> Result<RocState, JsError> {
...
/// Run the ROC indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[period]`
#[wasm_bindgen(js_name = "rocIndicator")]
pub fn roc_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Roc::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(RocState { inner }))
}
...
#[wasm_bindgen(js_name = "rocMinData")]
