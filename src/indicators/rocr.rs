...
use tulip_rs::indicators::rocr::{Indicator, IndicatorState, Rocr, INPUTS, OPTIONS};
...
impl RocrState {
    ...
    pub fn from_json(json: String) -> Result<RocrState, JsError> {
...
/// Run the ROCR indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[period]`
#[wasm_bindgen(js_name = "rocrIndicator")]
pub fn rocr_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Rocr::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(RocrState { inner }))
}
...
#[wasm_bindgen(js_name = "rocrMinData")]
