...
use tulip_rs::indicators::vwap::{Indicator, IndicatorState, Vwap, INPUTS, OPTIONS};
...
impl VwapState {
    ...
    pub fn from_json(json: String) -> Result<VwapState, JsError> {
...
/// Run the Volume Weighted Average Price indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[high], [low], [close], [volume]]` | `options`: `[]`
/// Mandatory outputs: `vwap` | Optional: `[want_typprice]`
#[wasm_bindgen(js_name = "vwapIndicator")]
pub fn vwap_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Vwap::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(VwapState { inner }))
}

/// Static metadata for VWAP.
#[wasm_bindgen(js_name = "vwapInfo")]
pub fn vwap_info() -> JsValue {
    info_to_object(Vwap::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "vwapMinData")]
pub fn vwap_min_data(options: Vec<f64>) -> u32 {
    let option_arr: [f64; OW] = options
        .try_into()
        .unwrap_or_else(|_| panic!("Expected {OW} options"));
    Vwap::min_data(&option_arr) as u32
}
