...
use tulip_rs::indicators::emv::{Emv, Indicator, IndicatorState, INPUTS, OPTIONS};
...
pub fn emv_indicator(inputs: JsValue, options: Vec<f64>, optional_outputs: JsValue) -> Result<js_sys::Array, JsError> {
    let inputs = inputs_from_js(inputs)?;
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Emv::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
...
pub fn emv_min_data(options: Vec<f64>) -> u32 {
    ...
    Emv::min_data(&option_arr) as u32
}
