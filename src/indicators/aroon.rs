...
use tulip_rs::indicators::aroon::{Aroon, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl AroonState {
    ...
    pub fn from_json(json: String) -> Result<AroonState, JsError> {
...
#[wasm_bindgen(js_name = "aroonIndicator")]
pub fn aroon_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Aroon::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(AroonState { inner }))
}

/// Static metadata for AROON.
#[wasm_bindgen(js_name = "aroonInfo")]
pub fn aroon_info() -> JsValue {
    info_to_object(Aroon::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "aroonMinData")]
