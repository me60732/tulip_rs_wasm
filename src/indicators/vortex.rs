...
use tulip_rs::indicators::vortex::{Indicator, IndicatorState, Vortex, INPUTS, OPTIONS};
...
impl VortexState {
    ...
    pub fn from_json(json: String) -> Result<VortexState, JsError> {
...
#[wasm_bindgen(js_name = "vortexIndicator")]
pub fn vortex_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Vortex::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(
        outputs_to_js(outputs)?,
        JsValue::from(VortexState { inner }),
    )
}

/// Static metadata for VORTEX.
#[wasm_bindgen(js_name = "vortexInfo")]
pub fn vortex_info() -> JsValue {
    info_to_object(Vortex::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "vortexMinData")]
