...
use tulip_rs::indicators::fosc::{Fosc, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl FoscState {
    ...
    pub fn from_json(json: String) -> Result<FoscState, JsError> {
...
#[wasm_bindgen(js_name = "foscIndicator")]
pub fn fosc_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Fosc::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(FoscState { inner }))
}

/// Static metadata for FOSC.
#[wasm_bindgen(js_name = "foscInfo")]
pub fn fosc_info() -> JsValue {
    info_to_object(Fosc::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "foscMinData")]
