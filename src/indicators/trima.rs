...
use tulip_rs::indicators::trima::{Indicator, IndicatorState, Trima, INPUTS, OPTIONS};
...
impl TrimaState {
    ...
    pub fn from_json(json: String) -> Result<TrimaState, JsError> {
...
/// Run the TRIMA indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[period]`
#[wasm_bindgen(js_name = "trimaIndicator")]
pub fn trima_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Trima::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(TrimaState { inner }))
}
...
#[wasm_bindgen(js_name = "trimaMinData")]
