...
pub fn nvi_indicator(inputs: JsValue, optional_outputs: JsValue) -> Result<js_sys::Array, JsError> {
    let inputs = inputs_from_js(inputs)?;
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
...
pub fn nvi_min_data() -> u32 {
    rust_nvi::min_data(&[]) as u32
}

/// Minimum input bars needed to achieve a given decimal accuracy.
#[wasm_bindgen(js_name = "nviMinDataAccuracy")]
pub fn nvi_min_data_accuracy(decimals: u32) -> u32 {
    rust_nvi::min_data_accuracy(&[], decimals as usize) as u32
}
