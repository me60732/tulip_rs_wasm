...
use tulip_rs::indicators::obv::{Indicator, Obv, INPUTS, OPTIONS};
...
pub fn obv_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Obv::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
...
pub fn obv_min_data(options: Vec<f64>) -> u32 {
    ...
    Obv::min_data(&option_arr) as u32
}
