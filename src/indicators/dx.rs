...
use tulip_rs::indicators::dx::{Dx, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl DxState {
    ...
    pub fn from_json(json: String) -> Result<DxState, JsError> {
...
#[wasm_bindgen(js_name = "dxIndicator")]
pub fn dx_indicator(
    inputs: JsValue,
    options: Vec<f64>,
    optional_outputs: JsValue,
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Dx::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(DxState { inner }))
}
...
#[wasm_bindgen(js_name = "dxMinData")]
