...
use tulip_rs::indicators::mfi::{Indicator, IndicatorState, Mfi, INPUTS, OPTIONS};
...
impl MfiState {
    ...
    pub fn from_json(json: String) -> Result<MfiState, JsError> {
...
#[wasm_bindgen(js_name = "mfiIndicator")]
pub fn mfi_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Mfi::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(MfiState { inner }))
}
...
#[wasm_bindgen(js_name = "mfiMinData")]
