...
use tulip_rs::indicators::zlema::{Indicator, IndicatorState, INPUTS, OPTIONS, Zlema};
...
pub fn zlema_indicator(inputs: JsValue, options: Vec<f64>, optional_outputs: JsValue) -> Result<js_sys::Array, JsError> {
    let inputs = inputs_from_js(inputs)?;
    let input_arr: [&[f64]; IW] = inputs.iter().map(|v| v.as_slice()).collect::<Vec<_>>()
        .try_into().map_err(|_| JsError::new(&format!("Expected {IW} input series")))?;
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Zlema::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(ZlemaState { inner }))
}

#[wasm_bindgen(js_name = "zlemaInfo")]
pub fn zlema_info() -> JsValue { info_to_object(Zlema::INFO) }
