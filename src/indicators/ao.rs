...
use tulip_rs::indicators::ao::{Ao, Indicator, IndicatorState, INPUTS, OPTIONS};
...
pub fn ao_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Ao::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
