...
impl BopState {
    ...
    pub fn from_json(json: String) -> Result<BopState, JsError> {
...
pub fn bop_indicator(inputs: JsValue, optional_outputs: JsValue) -> Result<js_sys::Array, JsError> {
    let inputs = inputs_from_js(inputs)?;
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
...
#[wasm_bindgen(js_name = "bopMinDataAccuracy")]
