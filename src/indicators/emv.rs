...
impl EmvState {
    ...
    pub fn from_json(json: String) -> Result<EmvState, JsError> {
...
#[wasm_bindgen(js_name = "emvIndicator")]
pub fn emv_indicator(inputs: JsValue, optional_outputs: JsValue) -> Result<js_sys::Array, JsError> {
    let inputs = inputs_from_js(inputs)?;
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = rust_emv::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(EmvState { inner }))
}
...
#[wasm_bindgen(js_name = "emvMinData")]
pub fn emv_min_data() -> u32 {
    rust_emv::min_data(&[]) as u32
}

/// Minimum input bars needed to achieve a given decimal accuracy.
#[wasm_bindgen(js_name = "emvMinDataAccuracy")]
pub fn emv_min_data_accuracy(decimals: u32) -> u32 {
    rust_emv::min_data_accuracy(&[], decimals as usize) as u32
}
