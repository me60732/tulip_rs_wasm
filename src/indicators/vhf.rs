...
use tulip_rs::indicators::vhf::{Indicator, IndicatorState, Vhf, INPUTS, OPTIONS};
...
impl VhfState {
    ...
    pub fn from_json(json: String) -> Result<VhfState, JsError> {
...
/// Run the VHF indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[period]`
#[wasm_bindgen(js_name = "vhfIndicator")]
pub fn vhf_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Vhf::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(VhfState { inner }))
}

/// Static metadata for VHF.
#[wasm_bindgen(js_name = "vhfInfo")]
pub fn vhf_info() -> JsValue {
    info_to_object(Vhf::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "vhfMinData")]
