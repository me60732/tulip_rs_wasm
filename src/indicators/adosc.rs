...
use tulip_rs::indicators::adosc::{Adosc, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl AdoscState {
    ...
    pub fn from_json(json: String) -> Result<AdoscState, JsError> {
...
#[wasm_bindgen(js_name = "adoscIndicator")]
pub fn adosc_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Adosc::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(AdoscState { inner }))
}

/// Static metadata for ADOSC.
#[wasm_bindgen(js_name = "adoscInfo")]
pub fn adosc_info() -> JsValue {
    info_to_object(Adosc::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "adoscMinData")]
