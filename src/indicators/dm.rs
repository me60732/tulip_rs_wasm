...
use tulip_rs::indicators::dm::{Dm, Indicator, IndicatorState, INPUTS, OPTIONS};
...
impl DmState {
    ...
    pub fn from_json(json: String) -> Result<DmState, JsError> {
...
#[wasm_bindgen(js_name = "dmIndicator")]
pub fn dm_indicator(
    inputs: JsValue,
    options: Vec<f64>,
    optional_outputs: JsValue,
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = Dm::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(outputs_to_js(outputs)?, JsValue::from(DmState { inner }))
}

/// Static metadata for DM.
#[wasm_bindgen(js_name = "dmInfo")]
pub fn dm_info() -> JsValue {
    info_to_object(Dm::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[wasm_bindgen(js_name = "dmMinData")]
