...
impl WildersState {
    ...
    pub fn from_json(json: String) -> Result<WildersState, JsError> {
...
/// Run the WILDERS indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[period]`
#[wasm_bindgen(js_name = "wildersIndicator")]
pub fn wilders_indicator(
    ...
    let opt_outs = crate::utils::optional_outputs_from_js(optional_outputs)?;
    let (outputs, inner) = rust_wilders::indicator(&input_arr, &option_arr, opt_outs.as_deref())
        .map_err(|e| JsError::new(&format!("{e:?}")))?;
    make_pair(
        outputs_to_js(outputs)?,
        JsValue::from(WildersState { inner }),
    )
}
...
#[wasm_bindgen(js_name = "wildersMinData")]
...
#[wasm_bindgen(js_name = "wildersMinDataAccuracy")]
